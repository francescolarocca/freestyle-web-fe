...
import AddMurettoForm from "./AddMurettoForm"; // Importa il nuovo componente del form
import "../../home.css";
...
const Home = () => {
  ...
  const [showAddForm, setShowAddForm] = useState(false); // Stato per visualizzare il form
  ...
  return (
    <div className="body-rapho">
      ...
      {showAddForm ? (
        <AddMurettoForm setShowAddForm={setShowAddForm} />
      ) : (
        <div className="card-rapho">
          <h2>Seleziona un Muretto</h2>
          <select
            className="select-rapho mt-3"
            value={selectedMuretto}
            onChange={(e) => setSelectedMuretto(e.target.value)}
          >
          ...
          <button className="btnho btn-rapho mt-4" onClick={handleNext}>
            Avanti
          </button>

          <button
            className="btnho btn-rapho mt-4"
            onClick={() => setShowAddForm(true)} // Mostra il form per aggiungere un nuovo muretto
          >
            Aggiungi Muretto
          </button>
        </div>
